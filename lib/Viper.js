...
var proto = {
	...
	bootstrap: function() {
		...
		// Starting server
		that.log('Starting server ...'.cyan);
		var server = this.server = this.router.listen(that.config.port, function() {
			that.log(('Listening on port '+ server.address().port).green);
		});
	},
	...
	_initPlugins: function() {
		...
		function attachPlugin(plugin) {
			...
			if( pluginName ) {
				...
				if( that.config.disabledPlugins.indexOf( pluginName ) >= 0 ) {
					return false;
				}

				// Extend plugin config from main-config
			...
			// attach plugins interface to instance under pluginName
			that[pluginName] = plugin.interface;
