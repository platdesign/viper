...
proto.bootstrap = function() {
	...
	return that._bootstrap()
	.then(function() {
		// Starting the server
		return that.di.serviceInjector.invoke(function($server) {
			return $server.start();
		});
	})
...
proto._runConfigBlocks = function() {
	...
	var _async = [];
	this.di.providerInjector.provide('$_configAsync', function(handler) {
		_async.push(handler);
	});
	...
	.then(function() {
		return _async.reduce(function(promise, handler) {
			return promise.then(handler);
		}, Q(true));
	})

	.then(function() {
		that.di.providerInjector.remove('$_configAsync');
